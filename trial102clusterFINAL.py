...
USE_FIRST_TWO_THIRDS_FOR_SEED = True  
USE_FULL_FOR_LS = False                
...
#  RMSE in concentration space for ranking/reporting 
...
# Grid search (in true space; rank by conc-RMSE)
...
if __name__ == "__main__":
    ...
    assert_columns(cls, ["PatientID","Cluster"], "Clustering CSV")
    ...
    cluster_labels = sorted(cls["Cluster"].unique())
    ...
    for cl in cluster_labels:
        ids_in_cluster = cls.loc[cls["Cluster"] == cl, "PatientID"].dropna().tolist()
    ...
    if summaries:
        ...
        out.to_csv("cluster_fit_summary.csv", index=False)
        print("\nSaved cluster-wise results to cluster_fit_summary.csv")
        print(out)
    else:
        print("\nNo clusters produced results. Check logs for ID mismatches or data issues.")